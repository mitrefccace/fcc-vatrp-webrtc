...
/*//VATRP Notes: modifying comments from vanilla lib indicated by quadruple slash or 
the block quote style here.

All instances of the 'hide' bootstrap 3 class are replaced with the equivalent 'd-none' class
...
////else
server = "https://vatrpjanus.task3acrdemo.com:8089/janus";
						...
						janus.attach(
						{
							...
							success: function(pluginHandle) {
								...
								Janus.log("Plugin attached! (" + sipcall.getPlugin() + ", id=" + sipcall.getId() + ")");
									...
									$('#sipcall').removeClass('d-none').show();
									/*// $('#login').removeClass('d-none').show();
									...
									$('#start').removeAttr('disabled').html("Stop")
										...
										registerUsername();
										$('#displayUsername').html(dialerUsername)
									},
								...
								onmessage: function(msg, jsep) {
									...
									if(result && result["event"]) {
										...
										if(event === 'registered') {
											...
											$('#you').removeClass('d-none').show().text("Registered as '" + result["username"] + "'");
											// Enable buttons to call now
											if(!registered) {
												registered = true;
												masterId = result["master_id"];
												$('#server').parent().addClass('d-none').hide();
												$('#authuser').parent().addClass('d-none').hide();
												$('#displayname').parent().addClass('d-none').hide();
												$('#password').parent().addClass('d-none').hide();
												$('#register').parent().addClass('d-none').hide();
												$('#registerset').parent().addClass('d-none').hide();
											...
											//VATRP toastr.success("Call accepted!");
											sipcall.callId = callId;
								...
								onlocalstream: function(stream) {
									...
									if(sipcall.webrtcStuff.pc.iceConnectionState !== "completed" &&
										...
										if(spinner == null) {
											...
											//VATRP spinner = new Spinner({top:100}).spin(target);
										} else {
											//VATRP spinner.spin();
										}
									}
									...
									if(!videoTracks || videoTracks.length === 0) {
										...
										$('#myvideo').removeClass('d-none').show();
									}
								},
...
destroyed: function() {
	...
	/*//
	* Fetch login parameters from URL
	//*/

	var urlParams = new URLSearchParams(window.location.search);
	dialerUsername = urlParams.get('username');
	dialerPassword = urlParams.get('password');
	sipRegistrar = urlParams.get('sipRegistrar');
	sipIdentity = urlParams.get('sipIdentity');
	console.log("RECEIVED " + username + " " + password + " " + sipRegistrar + " " + sipIdentity);
	$('#server').val(sipRegistrar);
	$('#username').val(sipIdentity);
	$('#authuser').val(dialerUsername);
	$('#displayname').val(dialerUsername);
	$('#password').val(dialerPassword);
	//// End custom code
});

$(function addPhoneDigit() {
	$('.num-button').click(function() {
		$('#peer')[0].value= $('#peer')[0].value + $(this).html()
	});
} );

$(function removePhoneDigit() {
	$('#peer-delete').click(function() {
		$('#peer')[0].value= $('#peer')[0].value.slice(0, -1) 
	});
} );

$(function modalExit(){
	$("#modal-hangup").click(function() {
		doHangup();
		$('#call-modal').modal('hide');
	})
})
...
//THIS IS THE REGISTER TO SUBMIT TO JANUS
...
function doCall(ev) {
	...
	console.log("HANDLE INITIATED " + JSON.stringify(handle));
	...
	else if(username.indexOf("sip:") != 0 || username.indexOf("@") < 0) {
		////This peerURI behavior may need to be modified for linphone-compatible calls
		peerURI = "sip:" + username + "@" + sipRegistrar.substring(4)
		console.log('peerURI')
		console.log(peerURI)
		/*// bootbox.alert('Please insert a valid SIP address (e.g., sip:pluto@example.com)');
	...
	if(peerURI === undefined) {
		peerURI = $('#peer' + suffix).val()
	}
	actuallyDoCall(handle, peerURI, doVideo);
}
function actuallyDoCall(handle, uri, doVideo, referId) {
	console.log("USING HANDLE " + JSON.stringify(handle));
	handle.peer = uri;
	console.log("USING HANDLE " + JSON.stringify(handle));
	handle.createOffer(
	{
	...
	janus.attach(
	{
		...
		success: function(pluginHandle) {
			...
			onlocalstream: function(stream) {
				...
				$('#videos' + helperId).removeClass('d-none').show();
				...
				if(!videoTracks || videoTracks.length === 0) {
					...
					$('#myvideo' + helperId).removeClass('d-none').show();
				}
			},
			onremotestream: function(stream) {
				...
				if($('#remotevideo' + helperId).length === 0) {
					...
					$("#remotevideo" + helperId).bind("playing", function () {
						...
						if(this.videoWidth)
							$('#remotevideo' + helperId).removeClass('d-none').show();
